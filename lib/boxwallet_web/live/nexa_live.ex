defmodule BoxwalletWeb.NexaLive do
  ...
  require Logger
  alias Boxwallet.Coins.Nexa

  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Boxwallet.PubSub, "nexa:status")
      Nexa.Server.resume_polling()
    end

    # Seed initial state from GenServer
    server_state = Nexa.Server.get_state()

    socket =
      assign(socket,
        blockchain_is_synced: server_state.blockchain_is_synced,
        coin_name: "Nexa",
        coin_name_abbrev: Nexa.coin_name_abbrev(),
        coin_title:
          "Nexa (NEXA) — a next-generation, scalable blockchain for peer-to-peer electronic cash.",
        coin_description:
          "Nexa is a next-generation blockchain designed for massive scalability and instant transactions. Built on proven UTXO technology, Nexa uses a unique proof-of-work mining algorithm and supports native tokens and smart contracts. It aims to fulfill the original vision of peer-to-peer electronic cash that can scale to serve billions of users worldwide.",
        ...
        testnet_enabled: testnet_enabled?(Nexa),
        pruning_enabled: pruning_enabled?(Nexa),
        prune_size: get_prune_size(Nexa),
        receive_address: "",
        send_address: "",
        address_valid: :empty,
  ...
  def handle_info({:nexa_state, state_map}, socket) do
  ...
  def handle_event("switch_tab", %{"tab" => tab}, socket) do
    ...
    Nexa.Server.set_active_tab(tab)
  ...
  def handle_event("confirm_toggle_testnet", _params, socket) do
    ...
    conf_file = Nexa.get_conf_file_location()
  ...
  defp get_icon_state(name, assigns) do
    case name do
      ...
      :daemon ->
        ...
        %{name: "hero-face-smile", hint: hint, color: "text-nexayellow", state: state}
      ...
      :staking ->
        %{name: "hero-bolt", hint: "Mining", color: "text-nexayellow", state: :disabled}
    end
  end
  ...
  def render(assigns) do
    ...
    <!-- Error alert -->
      ...
      <div class="flex justify-center items-start gap-4">
        <.coin_sidebar color="text-nexayellow" active_tab={@active_tab} />
        <div class="card bg-base-200 w-full max-w-6xl shadow-xl shadow-nexayellow/30 p-8">
          <!-- Logo and title section -->
          <div class="flex flex-col md:flex-row items-start gap-6 mb-6">
            <img
              src={~p"/images/nexa_logo.png"}
              alt="#{@coin_name} logo"
