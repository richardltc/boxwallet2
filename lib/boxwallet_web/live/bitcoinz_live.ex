defmodule BoxwalletWeb.BitcoinzLive do
  ...
  def mount(_params, _session, socket) do
    if connected?(socket) do
      Phoenix.PubSub.subscribe(Boxwallet.PubSub, "bitcoinz:status")
      Bitcoinz.Server.resume_polling()
    end

    # Seed initial state from GenServer
    server_state = Bitcoinz.Server.get_state()

    socket =
      assign(socket,
        blockchain_is_synced: server_state.blockchain_is_synced,
        coin_name: "BitcoinZ",
        coin_name_abbrev: Bitcoinz.coin_name_abbrev(),
        coin_title:
          "BitcoinZ (BTCZ) — a community-driven cryptocurrency focused on decentralization and privacy.",
        coin_description:
          "BitcoinZ is a community-driven cryptocurrency that combines the best of Bitcoin and Zcash. It uses the Equihash proof-of-work algorithm, making it ASIC-resistant and GPU-mineable. BitcoinZ features optional zk-SNARK privacy, large 2 MB blocks, and a fair launch with no premine or ICO. It aims to be a truly decentralized digital currency accessible to everyone.",
        ...
        testnet_enabled: testnet_enabled?(Bitcoinz),
        pruning_enabled: pruning_enabled?(Bitcoinz),
        prune_size: get_prune_size(Bitcoinz),
        receive_address: "",
        send_address: "",
        address_valid: :empty,
  ...
  def handle_info({:bitcoinz_state, state_map}, socket) do
  ...
  def handle_event("switch_tab", %{"tab" => tab}, socket) do
    ...
    Bitcoinz.Server.set_active_tab(tab)
  ...
  def handle_event("confirm_toggle_testnet", _params, socket) do
    ...
    conf_file = Bitcoinz.get_conf_file_location()
  ...
  defp get_icon_state(name, assigns) do
    case name do
      ...
      :daemon ->
        ...
        %{name: "hero-face-smile", hint: hint, color: "text-btczgold", state: state}
      ...
      :staking ->
        %{name: "hero-bolt", hint: "Mining (Equihash)", color: "text-btczgold", state: :disabled}
    end
  end
  ...
  def render(assigns) do
    ...
    <Layouts.app flash={@flash}>
      ...
      <%= if @downloading do %>
        <div role="alert" class="alert alert-info mb-4">
          ...
          <span>Downloading and installing BitcoinZ... Please wait.</span>
        </div>
      <% end %>
    ...
    <!-- Fetching params in progress alert -->
      <%= if @fetching_params do %>
        <div role="alert" class="alert alert-info mb-4">
          ...
          <span>Downloading zkSNARK proving parameters (~911MB)... This may take a few minutes.</span>
        </div>
      <% end %>
    ...
    <!-- Fetching params success alert -->
      <%= if @fetching_params_complete do %>
        <div role="alert" class="alert alert-success mb-4">
          ...
          <span>Proving parameters downloaded successfully!</span>
        </div>
      <% end %>
    ...
    <!-- Fetching params error alert -->
      ...
      <div class="flex justify-center items-start gap-4">
        <.coin_sidebar color="text-btczgold" active_tab={@active_tab} />
        <div class="card bg-base-200 w-full max-w-6xl shadow-xl shadow-btczgold/30 p-8">
          <!-- Logo and title section -->
          <div class="flex flex-col md:flex-row items-start gap-6 mb-6">
            <img
              src={~p"/images/btcz_logo.png"}
              alt="#{@coin_name} logo"
          ...
          <%= case @active_tab do %>
            ...
            <% :settings -> %>
              ...
              />
            <% :transactions -> %>
              <.coin_transactions color="text-btczgold" coin_daemon_started={@coin_daemon_started} transactions={@transactions} />
            <% :receive -> %>
              <.coin_receive color="text-btczgold" coin_daemon_started={@coin_daemon_started} receive_address={@receive_address} />
            <% :send -> %>
              <.coin_send color="text-btczgold" coin_daemon_started={@coin_daemon_started} coin_name_abbrev={@coin_name_abbrev} address_valid={@address_valid} send_address={@send_address} />
            <% _ -> %>
              <.coin_transactions color="text-btczgold" coin_daemon_started={@coin_daemon_started} transactions={@transactions} />
          <% end %>
        </div>
