...
defmodule Boxwallet.Coins.Litecoin do
  ...
  @home_dir_win "Litecoin"

  @core_version "0.21.4"
  def core_version, do: @core_version
  @download_file_arm32 "litecoin-" <> @core_version <> "-arm-linux-gnueabihf.tar.gz"
  @download_file_arm64 "litecoin-" <> @core_version <> "-aarch64-linux-gnu.tar.gz"
  @download_file_linux "litecoin-" <> @core_version <> "-x86_64-linux-gnu.tar.gz"
  ...
  @cli_file_lin "litecoin-cli"
  @cli_file_win "litecoin-cli.exe"
  @daemon_file_lin "litecoind"
  @daemon_file_win "litecoind.exe"
  ...
  def get_block_height() do
    ...
    case Req.get(url) do
      {:ok, %{status: 200, body: body}} when is_integer(body) ->
        Logger.info("[#{@coin_name_abbrev}] Block height: #{body}")
        {:ok, body}
  ...
  def load_wallet(auth) do
    ...
    Logger.info("[#{@coin_name_abbrev}] Attempting to load wallet")
  ...
  def create_wallet(auth) do
    ...
    Logger.info("[#{@coin_name_abbrev}] Attempting to create wallet")
