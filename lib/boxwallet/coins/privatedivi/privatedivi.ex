...
defmodule Boxwallet.Coins.PrivateDivi do
  ...
  @conf_file "divi.conf"
  @cli_file_lin "privatedivi-cli"
  @cli_file_win "privatedivi-cli.exe"
  @daemon_file_lin "privatedivid"
  @daemon_file_win "privatedivid.exe"

  # divi.conf file constants
  @rpc_user "privatedivirpc"
  @rpc_port "52473"
  ...
  def download_coin() do
    ...
    _sys_info = to_string(:erlang.system_info(:system_architecture))
      ...
      case HTTPoison.post(url, body, headers) do
        {:ok, %{body: response_body}} ->
          ...
          if String.contains?(response_body, "Loading") ||
            ...
            # Now we need to convert into a GetInfo before returning it to the UI
            case BoxWallet.Coins.PrivateDivi.GetInfo.from_json(response_body) do
              {:ok, response} ->
      ...
      case HTTPoison.post(url, body, headers) do
        {:ok, %{body: response_body}} ->
          ...
          if String.contains?(response_body, "Loading") ||
            ...
            case BoxWallet.Coins.PrivateDivi.GetBlockchainInfo.from_json(response_body) do
              {:ok, response} ->
                # Process the successful response - Halt with result
                {:halt, {:ok, response}}
      ...
      case HTTPoison.post(url, body, headers) do
        {:ok, %{body: response_body}} ->
          ...
          if String.contains?(response_body, "Loading") ||
            ...
            # Now we need to convert into a GetBlockchainInfo before returning it to the UI
            case BoxWallet.Coins.PrivateDivi.GetMNSyncStatus.from_json(response_body) do
              {:ok, response} ->
                # Process the successful response - Halt with result
                {:halt, {:ok, response}}
      ...
      case HTTPoison.post(url, body, headers) do
        {:ok, %{body: response_body}} ->
          ...
          if String.contains?(response_body, "Loading") ||
            ...
            # Now we need to convert into a GetBlockchainInfo before returning it to the UI
            case BoxWallet.Coins.PrivateDivi.GetWalletInfo.from_json(response_body) do
              {:ok, response} ->
                # Process the successful response - Halt with result
                {:halt, {:ok, response}}
