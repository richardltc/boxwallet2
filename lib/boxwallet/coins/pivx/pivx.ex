defmodule Boxwallet.Coins.Pivx do
  require Logger

  @coin_name "PIVX"
  @coin_name_abbrev "PIVX"
  ...
  @download_file_arm64 "pivx-" <> @core_version <> "-aarch64-linux-gnu.tar.gz"
  @download_file_linux "pivx-" <> @core_version <> "-x86_64-linux-gnu.tar.gz"
  @download_file_mac_arm64 "pivx-" <> @core_version <> "-osx64.tar.gz"
  @download_file_mac_x86 "pivx-" <> @core_version <> "-osx64.tar.gz"
  @download_file_windows "pivx-" <> @core_version <> "-win64.zip"

  @extracted_dir_linux "pivx-" <> @core_version
  @extracted_dir_windows "pivx-" <> @core_version

  # https://github.com/PIVX-Project/PIVX/releases/download/v5.6.1/pivx-5.6.1-x86_64-linux-gnu.tar.gz
  @download_url "https://github.com/PIVX-Project/PIVX/releases/download/v" <> @core_version <> "/"

  @conf_file "pivx.conf"
  @cli_file_lin "pivx-cli"
  @cli_file_win "pivx-cli.exe"
  @daemon_file_lin "pivxd"
  @daemon_file_win "pivxd.exe"
  @install_params "install-params.sh"
  ...
  defp copy_extracted_files() do
    ...
    # Copy install-params.sh for later execution (at root of extracted dir, not in bin/)
    full_path_install_params =
      Path.join([BoxWallet.App.home_folder(), extracted_dir, @install_params])

    dest_path_install_params =
      Path.join([BoxWallet.App.home_folder(), @install_params])

    if File.exists?(full_path_install_params) do
      Logger.info("[#{@coin_name_abbrev}] Copying #{@install_params}")
      File.cp!(full_path_install_params, dest_path_install_params)
      File.chmod!(dest_path_install_params, 0o755)
    else
      Logger.warning("[#{@coin_name_abbrev}] #{@install_params} not found in extracted files")
    end

    Logger.info("[#{@coin_name_abbrev}] Files copied and permissions set successfully.")
  end

  def run_install_params() do
    script_path = Path.join([BoxWallet.App.home_folder(), @install_params])
    ...
    unless File.exists?(script_path) do
      ...
      case System.cmd(cmd, args, stderr_to_stdout: true) do
        {output, 0} ->
          Logger.info("[#{@coin_name_abbrev}] #{@install_params} completed successfully")
          Logger.debug("[#{@coin_name_abbrev}] #{@install_params} output: #{output}")
          :ok

        {output, exit_code} ->
          Logger.error(
            "[#{@coin_name_abbrev}] #{@install_params} failed with exit code #{exit_code}: #{output}"
          )

          {:error, "install-params.sh failed with exit code #{exit_code}"}
      end
    end
  end
