defmodule Boxwallet.Coins.Pivx.Server do
  use GenServer
  require Logger
  ...
  @impl true
  def init(:ok) do
    coin_auth = Pivx.get_auth_values()
    coin_files_exist = Pivx.files_exist()
    ...
    state =
      case coin_auth do
        {:ok, auth} ->
          if Pivx.daemon_is_running(auth) do
            Logger.info("[PIVX] PIVX daemon already running on init")
            schedule_polls()
  ...
  def handle_cast(:start_daemon, state) do
    Pivx.start_daemon()
    Logger.info("[PIVX] PIVX daemon starting...")
  ...
  def handle_cast(:pause_polling, state) do
    Logger.info("[PIVX] PIVX polling paused")
  ...
  def handle_cast(:resume_polling, state) do
    ...
    if state.daemon_status in [:starting, :running] do
      Logger.info("[PIVX] PIVX polling resumed")
      schedule_polls()
    end
  ...
  def handle_info(:poll_blockchain_info, %{daemon_status: status, polling_paused: false} = state)
    ...
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Pivx.get_blockchain_info(auth)
  ...
  def handle_info(:poll_wallet_info, %{daemon_status: status, polling_paused: false} = state)
    ...
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Pivx.get_wallet_info(auth)
  ...
  def handle_info({:blockchain_info_result, {:error, _reason}}, state) do
    Logger.warning("[PIVX] PIVX blockchain info poll failed, retrying...")
  ...
  def handle_info({:wallet_info_result, {:error, _reason}}, state) do
    Logger.warning("[PIVX] PIVX wallet info poll failed, retrying...")
    maybe_reschedule(state, :poll_wallet_info, 2_000)
    {:noreply, state}
  end

  def handle_info({:daemon_stop_result, {:ok, _response}}, state) do
    Logger.info("[PIVX] PIVX daemon stopped successfully")
  ...
  def handle_info({:daemon_stop_result, {:error, reason}}, state) do
    Logger.error("[PIVX] Failed to stop PIVX daemon: #{inspect(reason)}")
  ...
  def handle_info({:download_result, {:ok}}, state) do
    Logger.info("[PIVX] PIVX download completed successfully, starting install-params...")
    coin_auth = Pivx.get_auth_values()
    ...
    # Run install-params.sh in the background
    server = self()

    spawn(fn ->
      result = Pivx.run_install_params()
  ...
  def handle_info({:download_result, {:error, reason}}, state) do
    Logger.error("[PIVX] PIVX download failed: #{inspect(reason)}")
  ...
  def handle_info({:fetch_params_result, :ok}, state) do
    Logger.info("[PIVX] install-params.sh completed successfully")
  ...
  def handle_info({:fetch_params_result, {:error, reason}}, state) do
    Logger.error("[PIVX] install-params.sh failed: #{inspect(reason)}")

    state = %{
      state
      | fetching_params: false,
        fetching_params_complete: false,
        fetching_params_error: "Failed to download sapling parameters: #{inspect(reason)}"
    }
  ...
  defp broadcast(state) do
    ...
    Phoenix.PubSub.broadcast(Boxwallet.PubSub, "pivx:status", {:pivx_state, payload})
  end
end
