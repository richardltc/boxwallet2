defmodule Boxwallet.Coins.Nexa.Server do
  use GenServer
  require Logger
  ...
  @impl true
  def init(:ok) do
    coin_auth = Nexa.get_auth_values()
    coin_files_exist = Nexa.files_exist()
    ...
    state =
      case coin_auth do
        {:ok, auth} ->
          if Nexa.daemon_is_running(auth) do
            Logger.info("[NEXA] Nexa daemon already running on init")
            schedule_polls()
  ...
  def handle_cast(:start_daemon, state) do
    Nexa.start_daemon()
    Logger.info("[NEXA] Nexa daemon starting...")
  ...
  def handle_cast(:pause_polling, state) do
    Logger.info("[NEXA] Nexa polling paused")
  ...
  def handle_cast(:resume_polling, state) do
    ...
    if state.daemon_status in [:starting, :running] do
      Logger.info("[NEXA] Nexa polling resumed")
      schedule_polls()
    end
  ...
  def handle_info(:poll_blockchain_info, %{daemon_status: status, polling_paused: false} = state)
    ...
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Nexa.get_blockchain_info(auth)
  ...
  def handle_info(:poll_wallet_info, %{daemon_status: status, polling_paused: false} = state)
    ...
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Nexa.get_wallet_info(auth)
  ...
  def handle_info({:blockchain_info_result, {:error, _reason}}, state) do
    Logger.warning("[NEXA] Nexa blockchain info poll failed, retrying...")
  ...
  def handle_info({:wallet_info_result, {:error, _reason}}, state) do
    Logger.warning("[NEXA] Nexa wallet info poll failed, retrying...")
    maybe_reschedule(state, :poll_wallet_info, 2_000)
    {:noreply, state}
  end

  def handle_info({:daemon_stop_result, {:ok, _response}}, state) do
    Logger.info("[NEXA] Nexa daemon stopped successfully")
  ...
  def handle_info({:daemon_stop_result, {:error, reason}}, state) do
    Logger.error("[NEXA] Failed to stop Nexa daemon: #{inspect(reason)}")
  ...
  def handle_info({:download_result, {:ok}}, state) do
    Logger.info("[NEXA] Nexa download completed successfully")
    coin_auth = Nexa.get_auth_values()
  ...
  def handle_info({:download_result, {:error, reason}}, state) do
    Logger.error("[NEXA] Nexa download failed: #{inspect(reason)}")
  ...
  defp broadcast(state) do
    ...
    Phoenix.PubSub.broadcast(Boxwallet.PubSub, "nexa:status", {:nexa_state, payload})
  end
end
