defmodule Boxwallet.Coins.Bitcoinz.Server do
  use GenServer
  require Logger

  alias Boxwallet.Coins.Bitcoinz
  ...
  @impl true
  def init(:ok) do
    coin_auth = Bitcoinz.get_auth_values()
    coin_files_exist = Bitcoinz.files_exist()
    ...
    state = %{
      ...
      download_error: nil,
      fetching_params: false,
      fetching_params_complete: false,
      fetching_params_error: nil,
      active_tab: :home,
    ...
    state =
      case coin_auth do
        {:ok, auth} ->
          if Bitcoinz.daemon_is_running(auth) do
            Logger.info("[BTCZ] BitcoinZ daemon already running on init")
            schedule_polls()
  ...
  def handle_cast(:start_daemon, state) do
    Bitcoinz.start_daemon()
    Logger.info("[BTCZ] BitcoinZ daemon starting...")
  ...
  def handle_cast(:stop_daemon, state) do
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Bitcoinz.stop_daemon(auth)
  ...
  def handle_cast(:pause_polling, state) do
    Logger.info("[BTCZ] BitcoinZ polling paused")
  ...
  def handle_cast(:resume_polling, state) do
    ...
    if state.daemon_status in [:starting, :running] do
      Logger.info("[BTCZ] BitcoinZ polling resumed")
      schedule_polls()
    end
  ...
  def handle_info(:poll_blockchain_info, %{daemon_status: status, polling_paused: false} = state)
    ...
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Bitcoinz.get_blockchain_info(auth)
  ...
  def handle_info(:poll_wallet_info, %{daemon_status: status, polling_paused: false} = state)
    ...
    case state.coin_auth do
      {:ok, auth} ->
        ...
        spawn(fn ->
          result = Bitcoinz.get_wallet_info(auth)
  ...
  def handle_info({:blockchain_info_result, {:ok, response}}, state) do
    ...
    if state.daemon_status == :running do
  ...
  def handle_info({:blockchain_info_result, {:error, _reason}}, state) do
    Logger.warning("[BTCZ] BitcoinZ blockchain info poll failed, retrying...")
  ...
  def handle_info({:wallet_info_result, {:ok, response}}, state) do
    ...
    wallet_encryption_status =
      cond do
        !Map.has_key?(result, "unlocked_until") -> :wes_unencrypted
        result["unlocked_until"] == 0 -> :wes_locked
        true -> :wes_unlocked
      end
  ...
  def handle_info({:wallet_info_result, {:error, _reason}}, state) do
    Logger.warning("[BTCZ] BitcoinZ wallet info poll failed, retrying...")
    maybe_reschedule(state, :poll_wallet_info, 2_000)
    {:noreply, state}
  end

  def handle_info({:daemon_stop_result, {:ok, _response}}, state) do
    Logger.info("[BTCZ] BitcoinZ daemon stopped successfully")
  ...
  def handle_info({:daemon_stop_result, {:error, reason}}, state) do
    Logger.error("[BTCZ] Failed to stop BitcoinZ daemon: #{inspect(reason)}")
  ...
  def handle_info({:download_result, {:ok}}, state) do
    Logger.info("[BTCZ] BitcoinZ download completed successfully, starting fetch-params...")
    coin_auth = Bitcoinz.get_auth_values()
    ...
    # Run fetch-params.sh in the background
    server = self()

    spawn(fn ->
      result = Bitcoinz.run_fetch_params()
      send(server, {:fetch_params_result, result})
    end)
  ...
  def handle_info({:download_result, {:error, reason}}, state) do
    Logger.error("[BTCZ] BitcoinZ download failed: #{inspect(reason)}")
  ...
  def handle_info({:fetch_params_result, :ok}, state) do
    Logger.info("[BTCZ] fetch-params.sh completed successfully")

    state = %{
      state
      | fetching_params: false,
        fetching_params_complete: true,
        fetching_params_error: nil
    }

    broadcast(state)
    Process.send_after(self(), :clear_fetch_params_success, 5_000)
    {:noreply, state}
  end

  def handle_info({:fetch_params_result, {:error, reason}}, state) do
    Logger.error("[BTCZ] fetch-params.sh failed: #{inspect(reason)}")

    state = %{
      state
      | fetching_params: false,
        fetching_params_complete: false,
        fetching_params_error: "Failed to download proving parameters: #{inspect(reason)}"
    }
  ...
  def handle_info(:clear_fetch_params_success, state) do
    state = %{state | fetching_params_complete: false}
    broadcast(state)
    {:noreply, state}
  end
  ...
  defp broadcast(state) do
    payload = %{
      ...
      fetching_params: state.fetching_params,
      fetching_params_complete: state.fetching_params_complete,
      fetching_params_error: state.fetching_params_error,
      disk_used_bytes: state.disk_used_bytes,
      disk_total_bytes: state.disk_total_bytes
    }

    Phoenix.PubSub.broadcast(Boxwallet.PubSub, "bitcoinz:status", {:bitcoinz_state, payload})
  end
end
