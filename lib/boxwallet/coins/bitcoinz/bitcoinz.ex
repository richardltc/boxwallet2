defmodule Boxwallet.Coins.Bitcoinz do
  require Logger

  @coin_name "BitcoinZ"
  @coin_name_abbrev "BTCZ"
  def coin_name_abbrev, do: @coin_name_abbrev

  @home_dir_lin ".bitcoinz"
  @home_dir_mac "BitcoinZ"
  @home_dir_win "BITCOINZ"

  @core_version "2.2.0"
  @core_hash "e90047d4ae65"
  def core_version, do: @core_version
  @download_file_arm64 "bitcoinz-" <> @core_hash <> "-aarch64-linux-gnu.tar.gz"
  @download_file_linux "bitcoinz-" <> @core_hash <> "-x86_64-linux-gnu.tar.gz"
  @download_file_mac_arm64 "bitcoinz-" <> @core_hash <> "-arm64-apple-darwin.tar.gz"
  @download_file_mac_x86 "bitcoinz-" <> @core_hash <> "-x86_64-apple-darwin.tar.gz"
  @download_file_windows "bitcoinz-" <> @core_hash <> "-win64.zip"

  @extracted_dir_linux "bitcoinz-" <> @core_hash
  @extracted_dir_windows "bitcoinz-" <> @core_hash

  # https://github.com/btcz/bitcoinz/releases/download/2.2.0/bitcoinz-e90047d4ae65-x86_64-linux-gnu.tar.gz
  @download_url "https://github.com/btcz/bitcoinz/releases/download/" <>
                  @core_version <> "/"

  @conf_file "bitcoinz.conf"
  @cli_file_lin "bitcoinz-cli"
  @cli_file_win "bitcoinz-cli.exe"
  @daemon_file_lin "bitcoinzd"
  @daemon_file_win "bitcoinzd.exe"
  @fetch_params "fetch-params.sh"

  # bitcoinz.conf file constants
  @rpc_user "bitcoinzrpc"
  @rpc_port "1979"
  ...
  defp copy_extracted_files() do
    ...
    full_path_fetch_params =
      Path.join([BoxWallet.App.home_folder(), extracted_dir, "bin", @fetch_params])
    ...
    dest_path_fetch_params =
      Path.join([BoxWallet.App.home_folder(), @fetch_params])
    ...
    # Copy fetch-params.sh for later execution
    if File.exists?(full_path_fetch_params) do
      Logger.info("[#{@coin_name_abbrev}] Copying #{@fetch_params}")
      File.cp!(full_path_fetch_params, dest_path_fetch_params)
      File.chmod!(dest_path_fetch_params, 0o755)
    else
      Logger.warning("[#{@coin_name_abbrev}] #{@fetch_params} not found in extracted files")
    end

    Logger.info("[#{@coin_name_abbrev}] Files copied and permissions set successfully.")
  end

  def run_fetch_params() do
    script_path = Path.join([BoxWallet.App.home_folder(), @fetch_params])
    ...
    unless File.exists?(script_path) do
      ...
      {cmd, args} =
        case :os.type() do
          {:win32, _} -> {"cmd.exe", ["/C", script_path]}
          _ -> {script_path, []}
        end

      case System.cmd(cmd, args, stderr_to_stdout: true) do
        {output, 0} ->
          Logger.info("[#{@coin_name_abbrev}] #{@fetch_params} completed successfully")
          Logger.debug("[#{@coin_name_abbrev}] #{@fetch_params} output: #{output}")
          :ok

        {output, exit_code} ->
          Logger.error(
            "[#{@coin_name_abbrev}] #{@fetch_params} failed with exit code #{exit_code}: #{output}"
          )

          {:error, "fetch-params.sh failed with exit code #{exit_code}"}
      end
    end
  end
      ...
      {:ok, %{body: response_body}} ->
        cond do
          String.contains?(response_body, "Rewinding") ->
            Logger.info("[#{@coin_name_abbrev}] Daemon is rewinding blocks")
      ...
      case HTTPoison.post(url, body, headers) do
        {:ok, %{body: response_body}} ->
          if String.contains?(response_body, "Loading") ||
               ...
               String.contains?(response_body, "Calculating money supply") do
            Logger.info(
